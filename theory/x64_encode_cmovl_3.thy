...
lemma encode_cmovl_2_subgoal_1 : "\<not> bit (v::u8) 3 \<Longrightarrow> \<not> bit v (Suc 0) \<Longrightarrow> \<not> bit (v >> 2) 0 \<Longrightarrow> \<not> bit v 0
...
lemma encode_cmovl_2: "
    and 15 ((v::u8) >> 4) = 4 \<Longrightarrow> and 15 v \<noteq> 0 \<Longrightarrow>
  ...
  apply (unfold bitfield_insert_u8_def u8_of_bool_def Let_def;simp)
  apply (cases "bit (v >> 2) 0";cases "bit v 0"; simp_all)
  using encode_cmovl_2_subgoal_1 
  by simp
